...
- ⚠️ **串口输出**: U-Boot SPL 阶段仍有部分乱码（SPL 使用硬编码的 24MHz 时钟），但内核和 U-Boot 主程序已配置为 115200，可以正常使用
- ⚠️ **设备树**: 仅完善了必要的基础设备树配置


## 串口配置

使用串口调试工具连接设备时，建议设置波特率为 **115200**：
...
**注意**: U-Boot SPL 阶段的早期输出可能出现乱码，这是正常现象，不影响内核启动后的正常使用。
